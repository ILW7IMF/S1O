...
Sonic_Main:	; Routine 0
		...
		move.b	#9,obWidth(a0)
		move.l	(v_characterassets+Asset_SonicMaps).w,obMap(a0)	; load character mappings
...
Sonic_Balance:
		...
		bne.s	loc_12F62           ;To do: make directional balancing anims
...
Sonic_Roll:
		...
		bpl.s	.ispositive
		neg.w	d0
.ispositive:	cmpi.w	#$100,d0	; is Sonic moving at $100 speed or faster?
...
Sonic_Animate:
		movea.l	(v_characterassets+Asset_SonicAnim).w,a1	; load animation scripts
		movea.l	a1,a2						; copy for special animations
		moveq	#0,d0
		move.b	obAnim(a0),d0
...
.nomodspeed:
		move.w	id_Run<<1(a2),d3			; use running animation
		lea	(a2,d3.w),a1
		cmpi.w	#$600,d2				; is Sonic at running speed?
		bhs.s	.running				; if yes, branch

		move.w	id_Walk<<1(a2),d3			; use walking animation
		lea	(a2,d3.w),a1
		move.b	d0,d1
		lsr.b	#1,d1
		add.b	d1,d0
...
.nomodspeed2:
		move.w	id_Roll2<<1(a2),d3			; use fast rolling animation
		lea	(a2,d3.w),a1
		cmpi.w	#$600,d2				; is Sonic moving fast?
		bhs.s	.rollfast				; if yes, branch

		move.w	id_Roll<<1(a2),d3			; use slower rolling animation
		lea	(a2,d3.w),a1
.rollfast:
		neg.w	d2
		addi.w	#$400,d2
...
.belowmax3:
		lsr.w	#6,d2
		move.b	d2,obTimeFrame(a0) ; modify frame duration
		move.w	id_Push<<1(a2),d3			; use push animation
		lea	(a2,d3.w),a1
		move.b	obStatus(a0),d1
		andi.b	#1,d1
		andi.b	#$FC,obRender(a0)
...
Sonic_LoadGfx:
		...
		movea.l	(v_characterassets+Asset_SonicDPLC).w,a2 ; load DPLC script
		move.w	#ArtTile_Sonic*tile_size,d4	; starting VRAM tile
		movea.l	(v_characterassets+Asset_SonicArt).w,a1 ; load uncompressed art
...
Sonic_AirRoll:
        ...
        btst    #1,obStatus(a0)   ; is Sonic airborn?
        beq.s   .return           ; Branch if equal

		moveq	#btnB,d0		; Check if B..
		and.b	(v_jpadpress2),d0	; is pressed?
        ...
        move.w  #sfx_Roll,d0  ; play rolling sound
        jsr     (QueueSound2).l ; ditto
